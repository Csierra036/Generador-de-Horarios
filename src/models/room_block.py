...
class RoomBlock(Base):
    ...
    block_id = Column(Integer, ForeignKey("block_class.id"), index=True)  # Relación con la tabla "bloque_clases"
    room_id = Column(Integer, ForeignKey("rooms.id"), index=True)  # Relación con la tabla "salones
